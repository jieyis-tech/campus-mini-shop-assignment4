...
- Search comparison is ASCII case-insensitive, which covers the English seed
  catalog. Query characters that have special meaning to SQL `LIKE`, including
  `%` and `_`, must be treated as literal characters rather than wildcards.
...
- A non-blank category value must match the stored category value exactly; it
  must use case-sensitive comparison, must not use substring matching, and must
  not silently map one category to another.
...
- Database category choices must be displayed in ascending case-sensitive
  order. If the query contains an unknown non-blank category, the control must
  include that submitted value as a temporary selected option so its preserved
  state is visible without adding it to the database.
...
- An effective checkout cart means at least one session-cart entry resolves to
  a current SQLite product and has a positive integer quantity. Boolean,
  fractional, string, non-positive, malformed, and deleted-product entries are
  ineffective and must not cause an unhandled error.
- Checkout and cart summaries contain only effective lines. Reading either
  summary must not silently repair or remove ineffective session entries. A
  successful checkout removes the entire `cart` key, including any ineffective
  entries; a validation or database failure leaves the original key unchanged.
...
- With a non-empty effective cart, all submitted fields must be validated in one
  ...
  authoritative cart summary, preserve the normalized (trimmed) submitted field
  values, and show a specific message beside every invalid field.
...
- The order row and all associated item rows must be inserted in one SQLite
  ...
  the cart must remain unchanged, and no partial order may remain. The request
  must return an understandable HTTP 500 response rather than redirecting to a
  confirmation page or exposing a raw database traceback.
...
| `total_cents` | Required positive integer equal to the saved item-total sum |
| `created_at` | Required text timestamp using SQLite `CURRENT_TIMESTAMP` (UTC `YYYY-MM-DD HH:MM:SS`) |
...
|---|---|
| `id` | Unique integer primary key generated by SQLite |
| `order_id` | Required reference to `orders.id` with `ON DELETE CASCADE` |
| `product_id` | Required identifier of the source product at checkout time; not a foreign key |
...
- SQLite foreign-key enforcement must be enabled for application database
  connections.
- `product_id` deliberately has no foreign-key constraint: order snapshots must
  remain readable if a source product is later removed. No product or order
  deletion interface is part of this extension.
...
1. A mixed-case partial `q` value returns every product whose name contains it
   ...
   category; `%` and `_` are verified as literal query characters rather than
   SQL wildcards.
...
3. Selecting one category displays only products whose stored category equals
   it exactly and case-sensitively; blank category displays all categories, and
   an unknown non-blank category displays the no-results state without an error
   while remaining visibly selected as a temporary option.
...
7. `GET /checkout` with an empty effective cart redirects to `/cart` with the
   ...
   creates no database rows. Malformed and deleted-product session entries are
   ignored safely and are not mutated merely by viewing cart or checkout.
...
11. The session cart is cleared only after the complete order transaction
    commits; a simulated database failure returns an understandable HTTP 500,
    leaves the cart unchanged, and leaves no partial order or item rows.
...
- [x] Migration and repeated-initialization expectations preserve base data and
  saved orders.
- [x] An independent requirements reviewer has checked this extension for
  ambiguity, missing cases, and unnecessary scope.
- [x] Approved review corrections have been incorporated before extension
